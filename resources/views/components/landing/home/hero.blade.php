    ...
    <div class="container-x relative pb-10 pt-24 sm:pt-28 lg:pb-14 lg:pt-32">
        <div class="grid gap-10 lg:grid-cols-12 lg:gap-8">
            <div class="lg:col-span-7">
                <a href="{{ C::yandexProfileUrl() }}" target="_blank" rel="noopener" class="cursor-pointer inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-xs font-medium text-white backdrop-blur transition hover:bg-white/15">
                    <span class="flex h-5 w-5 items-center justify-center rounded bg-[#FC3F1D] text-[10px] font-bold">Я</span>
                    <span class="font-bold">{{ C::yandexRating() }}</span>
                    ...
                    <div class="mt-5 flex flex-wrap items-center gap-x-5 gap-y-3">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div class="leading-tight">
                                <a href="tel:{{ C::phoneTel() }}" onclick="window.slomTrackCTA && window.slomTrackCTA('phone', 'hero_main')" class="mcn-phone cursor-pointer block font-display text-xl font-bold tracking-tight text-white hover:text-primary sm:text-2xl">{{ C::phoneDisplay() }}</a>
                        ...
                        @if (C::showWhatsApp() || C::showTelegram())
                            <div class="flex items-center gap-2 ">
                                @if (C::showWhatsApp())
                                    <a href="{{ C::messengerUrl(C::whatsappUrl(), 'hero') }}" target="_blank" rel="noopener" onclick="window.slomTrackCTA && window.slomTrackCTA('whatsapp', 'hero')" aria-label="Написать в WhatsApp" class="cursor-pointer flex h-11 w-11 items-center justify-center rounded-full bg-whatsapp text-whatsapp-foreground transition hover:brightness-110"><x-icons.lucide name="message-circle" class="h-5 w-5" /></a>
                                @endif
                                @if (C::showTelegram())
                                    <a href="{{ C::messengerUrl(C::telegramUrl(), 'hero') }}" target="_blank" rel="noopener" onclick="window.slomTrackCTA && window.slomTrackCTA('telegram', 'hero')" aria-label="Написать в Telegram" class="cursor-pointer flex h-11 w-11 items-center justify-center rounded-full bg-telegram text-telegram-foreground transition hover:brightness-110"><x-icons.lucide name="send" class="h-5 w-5" /></a>
                                @endif
                            </div>
                        @endif
                    </div>

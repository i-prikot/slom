        ...
        <div class="mt-12 grid gap-5 md:grid-cols-2">
            @foreach ($reviews as $review)
                <figure class="flex flex-col rounded-2xl border border-border bg-card p-6 shadow-card transition hover:-translate-y-0.5 hover:shadow-elevated">
                    ...
                    <div class="mt-4 flex items-center justify-between"><span class="flex items-center gap-0.5 text-primary" aria-hidden="true">@for ($i = 0; $i < 5; $i++)<svg viewBox="0 0 20 20" class="h-4 w-4 fill-current" focusable="false"><path d="M10 1.5l2.62 5.31 5.86.85-4.24 4.13 1 5.84L10 14.87l-5.24 2.76 1-5.84L1.52 7.66l5.86-.85L10 1.5z" /></svg>@endfor</span><span class="text-xs text-muted-foreground">{{ $review['date'] }}</span></div>

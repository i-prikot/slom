        ...
        <div class="grid gap-10 lg:grid-cols-12">
            <div class="lg:col-span-5">
                ...
                <div class="mt-5 flex flex-wrap items-center gap-2">
                    ...
                    <a href="{{ C::MAX_URL }}" target="_blank" rel="noopener" onclick="window.slomTrackCTA && window.slomTrackCTA('max', 'footer')" aria-label="Max" class="cursor-pointer flex h-12 w-12 items-center justify-center rounded-lg bg-max text-max-foreground hover:brightness-110"><x-icons.max class="h-5 w-5" /></a>
                </div>
            </div>
            <div class="lg:col-span-4">
                <div class="text-xs font-semibold uppercase tracking-wider text-white/50">Отзывы и рейтинг</div>
                <a href="{{ C::YANDEX_PROFILE_URL }}" target="_blank" rel="noopener" class="cursor-pointer mt-3 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 transition hover:border-primary/40 hover:bg-white/10">
                    <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-[#FC3F1D] font-bold text-white">Я</div>
                    <div class="flex-1"><div class="flex items-center gap-1.5"><span class="font-display text-xl font-bold leading-none text-white">{{ C::YANDEX_RATING }}</span><span class="flex gap-0.5 text-primary">@for ($i = 0; $i < 5; $i++)<x-icons.lucide name="star" class="h-3.5 w-3.5 fill-current" />@endfor</span></div><div class="text-xs text-white/60">Профиль на Яндекс.Картах</div></div>
            ...
            <div class="lg:col-span-3">
                ...
                <a href="{{ C::ADDRESS_MAP_URL }}" target="_blank" rel="noopener" class="cursor-pointer mt-3 flex items-start gap-2 text-sm text-white/80 transition hover:text-primary"><x-icons.lucide name="map-pin" class="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span>{{ C::ADDRESS }}</span></a>

        ...
        <div class="mt-8 overflow-hidden rounded-2xl bg-secondary p-6 text-secondary-foreground sm:p-8">
            <div class="grid grid-cols-1 items-center gap-5 lg:grid-cols-12">
                ...
                <div class="flex w-full flex-col gap-2.5 sm:flex-row lg:col-span-4 lg:flex-col lg:items-stretch xl:flex-row">
                    <a href="tel:{{ C::phoneTel() }}" onclick="window.slomTrackCTA && window.slomTrackCTA('phone', 'pricing')" class="mcn-phone cursor-pointer sm:flex-1 xl:flex-1"><span class="inline-flex h-14 w-full items-center justify-center gap-2 whitespace-nowrap rounded-xl bg-yellow-gradient px-8 text-base font-semibold text-primary-foreground shadow-cta transition hover:brightness-105 active:brightness-95"><x-icons.lucide name="phone" class="h-5 w-5" /> {{ C::phoneDisplay() }}</span></a>
                    @if (C::showWhatsApp() || C::showTelegram())
                        <div class="flex gap-2 sm:flex-1">
                            @if (C::showWhatsApp())
                                <a href="{{ C::messengerUrl(C::whatsappUrl(), 'pricing') }}" target="_blank" rel="noopener" onclick="window.slomTrackCTA && window.slomTrackCTA('whatsapp', 'pricing')" aria-label="WhatsApp" class="cursor-pointer flex h-14 flex-1 items-center justify-center rounded-xl bg-whatsapp text-whatsapp-foreground hover:brightness-110"><x-icons.lucide name="message-circle" class="h-5 w-5" /></a>
                            @endif
                            @if (C::showTelegram())
                                <a href="{{ C::messengerUrl(C::telegramUrl(), 'pricing') }}" target="_blank" rel="noopener" onclick="window.slomTrackCTA && window.slomTrackCTA('telegram', 'pricing')" aria-label="Telegram" class="cursor-pointer flex h-14 flex-1 items-center justify-center rounded-xl bg-telegram text-telegram-foreground hover:brightness-110"><x-icons.lucide name="send" class="h-5 w-5" /></a>
                            @endif
                        </div>
                    @endif
                </div>

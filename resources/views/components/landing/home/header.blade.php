...
<header class="absolute inset-x-0 top-0 z-30">
    <div class="container-x flex items-center justify-between gap-3 py-4 sm:py-5">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
            ...
            <a href="{{ C::MAX_URL }}" target="_blank" rel="noopener" onclick="window.slomTrackCTA && window.slomTrackCTA('max', 'header')" aria-label="Написать в Max" class="cursor-pointer hidden h-10 w-10 items-center justify-center rounded-md bg-max text-max-foreground hover:brightness-110 sm:inline-flex"><x-icons.max class="h-5 w-5" /></a>
            <a href="tel:{{ C::PHONE_TEL }}" onclick="window.slomTrackCTA && window.slomTrackCTA('phone', 'header_mobile')" class="cursor-pointer inline-flex h-10 items-center justify-center gap-2 rounded-md bg-yellow-gradient px-4 py-2 text-sm font-semibold text-primary-foreground shadow-cta transition hover:brightness-105 active:brightness-95 md:hidden"><x-icons.lucide name="phone" class="h-4 w-4" /> Позвонить</a>
        </div>
    </div>
</header>

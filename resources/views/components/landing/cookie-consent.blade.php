...
>
    <template x-if="mounted">
        <div
            role="dialog"
            aria-live="polite"
            aria-label="Согласие на использование cookie" class="pointer-events-none fixed inset-x-0 bottom-0 z-[60] px-3 pb-3 transition-all duration-500 ease-out sm:px-4 sm:pb-4"
        ...
        >
            <div class="pointer-events-auto mx-auto flex max-w-4xl flex-col items-start gap-3 rounded-xl border border-border bg-card p-4 text-card-foreground shadow-lg sm:flex-row sm:items-start sm:gap-4 sm:p-5">
                ...
                <p class="flex-1 text-sm leading-relaxed text-foreground/90">
                    ...
                    <a href="{{ route('terms') }}" class="cursor-pointer text-primary hover:underline" wire:navigate>Согласие на обработку</a>
                    персональных данных согласно нашей
                    <a href="{{ route('privacy') }}" class="cursor-pointer text-primary hover:underline" wire:navigate>Политике конфиденциальности</a>.
                </p>
                <button
                    type="button" class="cursor-pointer w-full shrink-0 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground sm:w-auto"
                    x-on:click="
